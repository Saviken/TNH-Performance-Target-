...
class PostViewSet(ModelViewSet):
    ...
    def approve(self, request, pk=None):
        ...
        from posts.models import Notification
        # Notify the owner (if available)
        # For now, notify all admin users as we don't have specific user assignment
        from django.contrib.auth import get_user_model
        User = get_user_model()

        # Check if Post has a branch for routing
        department_url = "/dashboard"  # default
        if hasattr(post, 'branch') and post.branch:
            department_url = get_department_route(post.branch.name)
